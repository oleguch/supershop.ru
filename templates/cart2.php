      ...
      <div class="cart-table-head">
        ...
        if (isset($_SESSION['cart_goods'])){
            foreach ($_SESSION['cart_goods'] as $item) {
                ...
                $numbers = $item['number'];
                $id_var = $item['var'];
                $res_good = pg_fetch_array(pg_query($dbconn, "SELECT goods.name, goods.price FROM goods WHERE id = ".$id_good));
                $name = $res_good['name'];
                if ($id_var != '') {
                    $var = pg_fetch_array(pg_query($dbconn, "SELECT variant FROM var_good WHERE id = ".$id_var))['variant'];
                    $name = $name." (".$var.")";
                }
                $img = pg_fetch_array(pg_query($dbconn, "SELECT images.img_path FROM images WHERE id_good = ".$id_good))['img_path'];
                $cena = $res_good['price'];
